...
import * as cc from './cyberchef'
import config from './config'
import statusBarTemplate from './statusbar.html'
import toolBarTemplate from './toolbar.html'
import './markdown-lint'
import '@hackmd/codemirror/addon/dialog/dialog'
import '@hackmd/codemirror/addon/display/panel'
import '@hackmd/codemirror/addon/search/searchcursor'
import '@hackmd/codemirror/addon/search/search'
import '@hackmd/codemirror/addon/search/jump-to-line'
import '@hackmd/codemirror/addon/search/matchesonscrollbar'
import '@hackmd/codemirror/addon/search/match-highlighter'
import './search-panel'
...
export default class Editor {
  ...
  addToolBar () {
    ...
    $(document).on('click', function (e) {
      if (!$(e.target).closest('#ccMenu').length) {
        $ccMenu.hide()
      }
    })

    $(document).on('click', '#ccMenu a[data-cc-op]', async (e) => {
      e.preventDefault()
      const op = e.currentTarget.dataset.ccOp
      $ccMenu.hide()
      const fn = cc[op]
      if (typeof fn !== 'function') return
      const cm = this.editor
      const input = cm.getSelection()
      if (!input) {
        if (window.toastr) toastr.info('Select text first')
        return
      }
      try {
        ...
        if (window.toastr) toastr.error('CyberChef: ' + err.message)
        else console.error('CyberChef:', err)
      }
    })
