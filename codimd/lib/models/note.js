...
module.exports = function (sequelize, DataTypes) {
  var Note = sequelize.define('Note', {
    ...
    hooks: {
      ...
      afterUpdate: function (note, options) {
        if (!note.alias) {
          logger.warn('afterUpdate: note has no alias, skipping filesystem write')
          return
        }
        ...
        // always write content to filesystem, creating the file if needed
        ...
        Note.findOne({
          ...
          if (note) {
            ...
            if (Note.checkFileExist(filePath)) {
              ...
              var body = fs.readFileSync(filePath, 'utf8').replace(/\r\n/g, '\n').replace(/\r/g, '\n')
  ...
  Note.writeNoteToFilesystem = function (note, filePath) {
    try {
      ...
      logger.info('writeNoteToFilesystem: wrote ' + content.length + ' bytes to ' + filePath)
      return true
    } catch (err) {
      logger.error('writeNoteToFilesystem: failed to write ' + filePath + ': ' + err.message)
      return false
    }
  }
