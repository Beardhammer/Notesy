...
async function parseNoteIdFromSocketAsync (socket) {
  ...
  // strip leading slashes from encoded URLs
...
// Extract username from FileBrowser JWT cookie (auth cookie on .scarla.tech domain)
function getFileBrowserUsername (socket) {
  try {
    var cookies = cookie.parse(get(socket, 'handshake.headers.cookie') || '')
    var token = cookies.auth
    if (!token) return null
    var parts = token.split('.')
    if (parts.length !== 3) return null
    var payload = JSON.parse(Buffer.from(parts[1], 'base64').toString('utf8'))
    return (payload.user && payload.user.username) ? payload.user.username : null
  } catch (err) {
    return null
  }
}
...
function updateUserData (socket, user) {
  ...
  if (socket.request.user && socket.request.user.logged_in) {
    ...
    // check for FileBrowser auth cookie
    var fbUsername = getFileBrowserUsername(socket)
    user.userid = null
    user.name = fbUsername || ('Guest ' + chance.animal())
    user.login = false
  }
}
