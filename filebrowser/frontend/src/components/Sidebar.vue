<template>
  <nav :class="{ active }">
    <template v-if="isLogged">
      ...
      <div>
        ...
        <button
          class="action"
          @click="toCyberChef"
          :aria-label="$t('sidebar.cyberchef')"
          :title="$t('sidebar.cyberchef')"
        >
          <i class="material-icons">science</i>
          <span>{{ $t("sidebar.cyberchef") }}</span>
        </button>
    ...
    <p class="credits">
      <span>
        <span v-if="disableExternal">Notesy McNotes Face</span>
        <a
          v-else
          rel="noopener noreferrer"
          target="_blank"
          href="https://github.com/filebrowser/filebrowser"
          >Notesy McNotes Face</a
        >
        <span> {{ version }}</span>
      </span>
      <span>
...
export default {
  ...
  methods: {
    ...
    toCyberChef() {
      this.$router.push({ path: "/cyberchef" }, () => {});
      this.$store.commit("closeHovers");
    },
    toSchedule() {
